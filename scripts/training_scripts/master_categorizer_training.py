...
### check to see if we should use weights for some previous model
give_weights = False
...
parser.add_argument('-gw', '--give_weights', type=lnn.str2bool, default=give_weights, help='If the weights from a previous run should be loaded')
parser.add_argument('-bw', '--use_base_weights', type=lnn.str2bool, default=use_base_weights, help='If the weights of a base model should be loaded with a classifier')
parser.add_argument('-bf', '--base_file_name', default=base_file_name, help='Filename of base model with weights')
...
give_weights = args.give_weights
use_base_weights = args.use_base_weights
base_file_name = args.base_file_name

luminosity_byproduct = 'category'
...
model2 = get_master_categorizer(modelLoc, pix_x, pix_y, numb_maps, get_master_res_next,
                train_number=0, file_name=file_name, base_file_name=base_file_name,
                give_weights=give_weights, use_base_weights=use_base_weights, **kwargs)
...
class LossHistory(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, batch, logs={}):
        ...
        self.mean_squared_error.append(logs.get('acc'))
