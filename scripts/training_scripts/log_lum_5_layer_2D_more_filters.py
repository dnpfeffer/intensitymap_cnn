...
fileName = 'log_lum_5_layer_2D_model_more_filters'
...
if make_model:
    ...
    ### convolutional layer
    model2.add(keras.layers.Conv2D(512, (5,5), activation='relu'))
    model2.add(keras.layers.BatchNormalization())
    ### use a convolution instead of a pool that acts like a pool
    model2.add(keras.layers.Conv2D(512, kernel_size=(2,2), strides=(2,2), activation='relu'))
    # model2.add(keras.layers.MaxPooling2D(pool_size=(2,2), strides=(2,2)))
    model2.add(keras.layers.Dropout(droprate))
