...
continue_training_model_loc = 'more_efficient_testing.hdf5'
...
fileName = 'more_efficient_testing'
...
dataset = dataset.map(lambda item: tuple(tf.py_func(lnn.utf8FileToMapAndLum, [item, 'basicL', True], [tf.float64, tf.float64])))
dataset = dataset.repeat()
dataset = dataset.batch(batch_size)

model2 = keras.utils.multi_gpu_model(model2, 2)
model2.compile(loss=keras.losses.msle,
                  optimizer=keras.optimizers.SGD(),
                  metrics=[keras.metrics.mse])
model2.summary()
